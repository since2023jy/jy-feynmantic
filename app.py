...
def get_map_system_prompt(role, topic, current_known):
    instruction = ""
    if role == "SCHOOL":
        instruction = "초등/중등 학생이 이해할 수 있도록 '피자', '서랍' 같은 구체적인 비유와 개념의 전제 조건을 'Unknown'으로 제시하십시오."
        role_persona = "친절한 가정 교사"
    elif role == "PRO":
        instruction = "기업 전문가(레드팀)의 관점에서, 사용자의 주장 중 '규제 리스크', '경쟁자 방어 논리', '수익성(ROI)' 관련 비즈니스 결함을 'Unknown'으로 제시하여 공격하십시오."
        role_persona = "냉철한 투자 심의 위원"
    elif role == "EXPLORER":
        instruction = "지적 탐험가로서, 해당 주제의 '역사적 맥락', '윤리적 딜레마', 또는 '다른 학문과의 연결고리' 같은 경계를 확장할 새로운 영역을 'Unknown'으로 제시하십시오."
        role_persona = "광장의 소크라테스"
    else:
        instruction = "일반적인 지식의 연결고리를 Unknown으로 제시하세요."
    ...
    try:
        ...
        if parsed: return parsed
            ...
            if retry_count < 1:
                ...
                return {"decision": "FAIL", "response": f"JSON Parsing Failed: {res.text}"}
...
with st.sidebar:
    ...
    st.caption("Persona Filter Lock")
...
if st.session_state.mode == "LANDING":
    ...
    st.caption("선택에 따라 AI의 질문 방향이 완전히 달라집니다.")
    ...
    if not st.session_state.auto_model:
        ...
        if st.button("🚩 깃발 꽂고 정복 시작"):
            if topic_input:
                ...
                st.session_state.territory = {"known": [], "unknown": []} # Reset map for new topic
                st.rerun()
...
# --- SCENE 3: CONQUEST (Map Building) ---
        ...
        if u_list:
            ...
            for i, u in enumerate(u_list):
                if cols[i%4].button(f"🔍 {u} 탐험하기", key=f"explore_{u}"):
                    st.session_state.messages.append({"role":"user", "content":f"'{u}'에 대해 더 알아서 내 지도를 넓히고 싶어. 이게 내가 아는 것({k_list})과 어떻게 연결돼?"})
                    st.rerun()
        else: st.caption("현재 더 탐험할 미지의 땅이 없습니다.")
    ...
    for msg in st.session_state.messages:
        ...
        st.markdown(f"<div classs='chat-message {css}'>{msg['content']}</div>", unsafe_allow_html=True)
    ...
    if st.session_state.messages and st.session_state.messages[-1]["role"] != "user" and st.session_state.messages[-2]["role"] == "user":
        ...
        with st.chat_message("assistant"):
            ...
            # [CRUCIAL] DYNAMIC SYSTEM PROMPT CALL
            ...
            # Map Update Logic (Guaranteed clean lists)
